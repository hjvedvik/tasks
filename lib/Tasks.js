...
const UpdateRenderer = require('./update-renderer/UpdateRenderer')
...
class Tasks {
  ...
  async run (context = {}, main = this) {
    ...
    const promises = pMap(this.tasks, async task => {
      ...
      // rerender and wait some milliseconds after a progressbar is finished
      if (task.progress) renderSleep()
    ...
    return promises.then(() => {
      context.totalTime = totalTimer(hirestime.S)
      return context
    })
  }
}
