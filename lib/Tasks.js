...
class Tasks {
   ...
   */
  async run (context = {}) {
    const { renderer, concurrent } = this.options
    ...
    const promises = pMap(this.tasks, async task => {
      ...
      if (!skip) {
        ...
        // set summary if it skip method returned a string
      ...
      // rerender and wait some milliseconds if task rendered
    ...
    return promises.then(() => {
      if (this.parent === this) {
        if (this.options.showSummary) {
          this.renderer.summary(this.tasks, {
   ...
   */
  log (force = false, skip = 150) {
    const { parent } = this

    if (parent._skippedRenders < skip && !force) {
      parent._skippedRenders++
      return
    }
